...
def stahni_cista_data(ticker):import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
...
# --- UŽIVATELSKÝ VSTUP ---
ticker = st.text_input("Zadejte ticker akcie (např. AAPL, NVDA, TSLA, F):", "AAPL").upper().strip()
...
if tlacitko:
    with st.spinner("AI optimalizuje matematické vztahy indikátorů pro dnešní den..."):
        ...
        close_prices = pd.Series(data['Close'].to_numpy().flatten(), index=data.index)Omlouvám se, do souboru app.py se na konec omylem dostal kousek mého vysvětlujícího textu z chatu, což způsobilo chybu SyntaxError.
...
Až predikci spustíte, dejte vědět, zda grafika i nová čísla z kalkulačky naskočily správně.
        ...
        fig_ind.add_trace(go.Scatter(x=data.index, y=data['Close'], name='Cena', line=dict(color='#1f77b4', width=2)), row=1, col=1)
        fig_ind.add_trace(go.Scatter(x=data.index, y=data['SMA20'], name='SMA 20', line=dict(color='#ff7f0e', dash='dash')), row=1, col=1)
Při použití kódu buďte obezřetní.fig_ind.add_trace(go.Scatter(x=data.index, y=data['SMA50'], name='SMA 50', line=dict(color='#d62728', dash='dot')), row=1, col=1)fig_ind.add_trace(go.Scatter(x=data.index, y=data['RSI'], name='RSI', line=dict(color='#9467bd')), row=2, col=1)fig_ind.add_hline(y=70, line_dash="dash", line_color="red", row=2, col=1)fig_ind.add_hline(y=30, line_dash="dash", line_color="green", row=2, col=1)fig_ind.add_trace(go.Scatter(x=data.index, y=data['MACD'], name='MACD', line=dict(color='#e377c2')), row=3, col=1)fig_ind.add_trace(go.Scatter(x=data.index, y=data['MACD_Signal'], name='Signál', line=dict(color='#bcbd22', width=1)), row=3, col=1)fig_ind.update_layout(height=650, template="plotly_white", showlegend=True, xaxis3_title="Datum")st.plotly_chart(fig_ind, use_container_width=True)
    d = yf.download(ticker, period="3y", interval="1d", multi_level_index=False)
    s = yf.download("^GSPC", period="3y", interval="1d", multi_level_index=False)
    v = yf.download("^VIX", period="3y", interval="1d", multi_level_index=False)
    return d, s, v
